...
- **The SOP binds dispatched subagents, not the interactive session.** [`docs/agent-task-workflow.md`](docs/agent-task-workflow.md) is the operating contract for *task agents*: they cannot consult the user mid-task, so the document must carry the entire procedure, conservatively. An interactive session working directly with the user follows the **user's live instructions** instead, and adopts SOP steps only when the user says to. (Rationale: a session that treats the SOP as self-binding while *editing* it recursively binds itself to its own edits — this happened, and it produced ritual compliance like gating markdown diffs.) What an interactive session always keeps is the physics and the hard lines, not the process: worktree isolation (above), the token guard + push-before-`pr create` mechanics, the **privacy gate** (the repo is public no matter who edits), and the PR conventions (merge-when-ready flag; approval is exclusively the user's). Docs-only diffs run no local gate — nothing in `make fix check` validates markdown; CI's required check covers branch protection.
...
- **Ambient self-state and n≤1 reachability (the perception invariants)**: Penny's internal store, state, and logs are present in her ambient space — the self-state header renders her operational situation (active mechanisms, recent runs/mutations/autonomous sends, the store map, the durable user core) deterministically from the registry and ledger. Those logs and ledgers are *system-managed*: written by the framework at chokepoints (the prompt log, the mutation ledger, the outgoing-message log), never curated by the model. From that ambient state, **no piece of operational information is more than one general tool call away, and the call never requires a guess**: every anchor (a name, a typed id, a key) is rendered verbatim on a surface the model reads, and the small general verb set (`read_similar`, `log_read`, `memory_metadata`, `find_mine`, …) consumes it. A required tool argument that no render emits is a reachability bug — guessing is n=∞ (in the motivating session, 17 of 21 tool errors were failed name guesses). Corollary: **metadata renders where the data renders** — a fact carried by a row the model already reads (provenance, status, origin) appears inline in that row's render, never behind a bespoke resolver verb; a resolver tool for a join the returned row already carries is a rendering bug wearing a tool costume (the `message_source` deletion on PR #1608 is the canonical counter-example), and the unmarked render case stays the quiet default. Enforcement is **design-time, on the ticket, by the ticket's designer**: any design touching a model-facing surface carries a *reachability sketch* — (a) the originating ambient state at the moment the information matters, and (b) the ≤1-call path (anchor → verb) from that state to each fact the design makes available — posted before implementation is dispatched. (Source: epic #1554's standing n≤1 invariant.)
