...
- **Trigger union (#1591/#1604)**: `collection_create` exposes an exclusive three-way trigger, parsed by `build_trigger` (`tools/collection_instantiation.py`): `interval` (a recurring cadence) | `run_at` + `max_runs` (the once-shaped schedule above) | `on_advance` (the name of a source LOG — the collection wakes when that log advances past its cursor, #1604; optional `min_interval` throttle floor). The on_advance form persists `memory.source_log` and is paced at `min_interval` or the dispatcher tick (its real gate is the source frontier, not the clock). The tool validates the source names an existing log-shaped memory (an actionable error otherwise). The trigger clause renders as `on advance of <log>` on the self-state mechanisms line (`SelfStateHeader._cadence`), in `memory_metadata`, and in the creation echo — so a source-driven mechanism reads at a glance, cadence collections stay byte-identical.
