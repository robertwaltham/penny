...
WHOLE-RENDER literal (pr-review-guide §6): the run header (identity · RESULT · gate), the folded
per-sample transcript, the multi-family rollup + per-case headings, the diff-mode flips index,
the local-artifacts footer, and the CLI contract.
"""
...
from penny.tests.eval.assemble import USAGE, assemble_run_comment, load_manifest, main
...
def _browse_sample(folded: bool) -> str:
    """A one-sample browse-answer transcript block (rendered by ``report.py``)."""
    events = [
    ...
    sample = report.build_sample(
        number=1, banner=banner, events=events, checks=checks, run_close_score="1/1", folded=folded
    )
...
def test_single_gated_case_whole_render(tmp_path: Path) -> None:
    ...
    folded clean-pass transcript, and the footer — no per-case heading (single case)."""
    ...
    _write_run(tmp_path, manifest, [artifact], {artifact.case_id: _browse_sample(folded=True)})
...
def test_diff_mode_flips_index_whole_render(
    ...
    run = tmp_path / "run"
    _write_run(run, manifest, [artifact], {artifact.case_id: _browse_sample(folded=True)})
